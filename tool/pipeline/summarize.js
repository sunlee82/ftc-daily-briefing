...
// │   - 위원회 소식(committee): PDF는 파일로만 저장(pdfPath), 요약은 안 함 │
// │   - 뉴스 보도내용(news): 검색 결과 제목·스니펫을 그대로 사용            │
// │  화면에서는 사용자가 원문을 보고 취사선택·순서조정만 하면 되고,        │
// │  제목/요약 다듬기·위원회 PDF 요약·최종 배포(git push)는 이 결과를      │
...
/**
 * 위원회 소식 PDF는 요약하지 않고 파일로만 저장해둔다(pdfPath). summary는
 * 자리표시자이며, 실제 [주요일정]/[인사발령] 추출은 이후 Claude Code가 pdfPath의
 * PDF를 직접 읽어 수행한다. pdfPath는 로컬 전용 필드로, 실제 배포 시에는 docs/data에
 * 포함되지 않는다(Claude Code가 최종 정리하며 제거).
 ...
 */
function buildRawCommitteeItems(rawItems, pdfDir = PDF_DIR) {
